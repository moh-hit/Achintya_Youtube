...
export default function YoutubeLiveView(props) {
  ...
  const [currentlyJoinedGroup, setCurrentlyJoinedGroup] = useState(0);
  ...
  function onSwiping({ dir }) {
    ...
    if (watchingOther) {
      if (dir == RIGHT) {
        ...
        if (currentGroup == 0) {
        } else setCurrentGroup(currentGroup - 1);
      } else if (dir == UP) {
        if (groupCount == currentGroup || groupCount == null) {
        } else setCurrentGroup(currentGroup + 1);
      }
    }
  }
  ...
  const exitOthersStream = () => {
    ...
    firebase.database().ref(`/${loggedUser}`).update({ space: "self" });
        ...
        {!joinAnother && watchingOther ? (
          ...
          >
            {currentGroup >= 1 ? (
              ...
              >
                <YoutubeComp videoId={groupsUser[currentIndex]} />
              </View>
            ) : (
              <>
                {userTurnVideo && userTurnVideo !== videoId ? (
                  <YoutubeComp
                    videoId={userTurnVideo}
                    opacity={watchCreator ? 0 : 1}
                  />
                ) : null}
                <YoutubeComp videoId={videoId} opacity={watchCreator ? 1 : 0} />
              </>
            )}
          ...
          (watchingOther && createGroup) ? (
          <>
            <YoutubeComp videoId={userVideos[videoIndex]} />
          ...
          <View
            style={{
              ...
              justifyContent: "center",
              alignItems: "center",
              display: joinAnother ? "none" : "flex",
            }}
          >
            <YoutubeComp videoId={selfVid} />
            <View
              style={{
                position: "absolute",
                top: 0,
